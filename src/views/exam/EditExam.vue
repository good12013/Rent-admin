  ...
  <a-card :body-style="{ padding: '24px 32px' }" :bordered="false">
    <a-form layout="horizontal" @submit="handleSubmit" :form="form" class="form">
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="测评名称">
        <a-input
          placeholder="请输入测评名称"
          v-decorator="['title', { rules: [{ required: true, message: '请输入名称' }], initialValue: exam.title }]"
        />
      </a-form-item>
      ...
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="测评分类">
        <a-tree-select
          ...
          v-decorator="[
            'category_id',
            { rules: [{ required: true, message: '请选择测评分类' }], initialValue: String(exam.category_id) }
          ]"
      ...
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="类别">
        <a-select
          ...
          v-decorator="[
            'is_company',
            { rules: [{ required: true, message: '请选择类别' }], initialValue: exam.is_company }
          ]"
        >
      ...
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="测评封面">
        <a-upload
          ...
          v-decorator="[
            'cover',
            {
              rules: [{ required: true, message: '请上测评封面' }],
              initialValue: exam.cover
            }
          ]"
        >
          <img width="128" height="128" v-if="exam.cover" :src="exam.cover" />
          <div v-else>
      ...
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="测评价格">
        <a-input
          ...
          v-decorator="[
            'price',
            { rules: [{ required: true, message: '请输入价格(单位：分)' }], initialValue: exam.price }
          ]"
          style="width: 150px"
        />
      ...
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="测评简介">
        <a-textarea
          ...
          v-decorator="[
            'content',
            { rules: [{ required: true, message: '请输入测评简介' }], initialValue: exam.content }
          ]"
...
import { updateExam, getExam } from '@/api/exam'
  ...
  methods: {
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          values.cover = _this.exam.cover
          updateExam(_this.$route.params.id, values).then(res => {
            if (res.code === 0) {
    ...
    handleCoverChange(info) {
      if (info.file.status === 'uploading') {
        ...
        this.exam.cover = `${this.uploadHost}/${this.ossParams.key}`
        this.loading = false
